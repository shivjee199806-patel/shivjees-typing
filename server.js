...
app.get('/api/admin/payment-system-control',auth,admin,(req,res)=>{res.set('Cache-Control','no-store');res.json({enabled:paymentSystemEnabled(),value:setting('payment_system_enabled')??'0'})});
app.put('/api/admin/payment-system-control',auth,admin,(req,res)=>{const enabled=!!(req.body||{}).enabled;db.prepare(`INSERT INTO site_settings(key,value) VALUES('payment_system_enabled',?) ON CONFLICT(key) DO UPDATE SET value=excluded.value`).run(enabled?'1':'0');audit(req,'UPDATE','payment_system','master',enabled?'ON':'OFF / FREE MODE');res.set('Cache-Control','no-store');res.json({ok:true,enabled,value:enabled?'1':'0'})});
