...
app.post('/api/auth/owner-otp-start',authRateLimit,async(req,res)=>{try{
 ...
 let delivered={dev:true};
 try{delivered=await deliverOtp(phone,code)}catch(otpErr){console.log(`[OWNER SCREEN OTP] ${phone}: ${code}`);delivered={dev:true}}
 const challenge=jwt.sign({ownerOtp:u.id,phone,otp_hash:otpHash(phone,'owner_login',code)},SECRET,{expiresIn:'5m'});
 res.json({challenge,phone_masked:maskPhone(phone),dev_otp:delivered?.dev?code:undefined});
