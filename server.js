...
function applyVerifiedExamDefaultsOnce(){
 ...
 const marker='exam_wise_behavior_defaults_v8_20260918';
 if(db.prepare('SELECT 1 FROM app_meta WHERE key=?').get(marker))return;

 // Broad/mixed folders are never assigned a guessed typing behaviour.  Only the
 // provenance flag changes here; existing Owner values are left intact because
 // Candidate Exam Mode is blocked while status=needs_rules.
 const markNeeds=db.prepare(`UPDATE exams SET official_rule_status='needs_rules',official_rule_note=?,official_rule_source='',official_rule_cycle='',evaluation_rule='generic' WHERE slug=? AND NOT (COALESCE(official_rule_status,'')='owner_configured_manual' OR (COALESCE(official_rule_status,'')='owner_configured' AND COALESCE(official_rule_note,'') LIKE 'Owner configured this exam profile.%'))`);
 const broadNote='Exact recruitment/cycle rules are not verified for this broad folder. Direct Exam Mode is disabled until Owner configures the exact notification. Duration, WPM, Backspace, Highlight and counting are not inferred from the folder name.';

 // Fields: slug, duration, WPM, accuracy, minWords, method, countMode,
 // backspaceMode, backspaceLimit, highlightMode, layout, wordMap,
 // evaluationRule, status, cycle, source, note.
 const verified=[
  ['upp-co-english',15,30,85,0,'wpm_accuracy','word','unlimited',0,'none','QWERTY','{"15":500}','upprpb_co','verified_mixed','UPPRPB Computer Operator Grade-A 2023 recruitment / typing notice 2026','https://uppbpb.gov.in/FilesUploaded/Notice/202601091832048444d73aac8-e395-4919-a953-876b8a868625.pdf','Official core: 15 min, 30 WPM, 85% accuracy, about 500-word English passage. Board PDF publishes the scoring rules; correction/highlight UI is kept as the observed simulator default and is not mislabelled as a Board-published UI rule.'],
  ['upp-co-hindi',15,25,85,0,'wpm_accuracy','word','unlimited',0,'none','Unicode / Mangal Inscript','{"15":400}','upprpb_co','verified_mixed','UPPRPB Computer Operator Grade-A 2023 recruitment / typing notice 2026','https://uppbpb.gov.in/FilesUploaded/Notice/202601091832048444d73aac8-e395-4919-a953-876b8a868625.pdf','Official core: 15 min, 25 WPM, 85% accuracy, about 400-word Hindi passage, Unicode Inscript. Board PDF publishes the scoring rules; correction/highlight UI is kept as the observed simulator default and is not mislabelled as a Board-published UI rule.'],
  ['ssc-chsl-ldc-jsa-typing',10,35,0,0,'wpm','character','unlimited',0,'none','QWERTY','{}','ssc_chsl','verified_core','SSC CHSL LDC/JSA Typing Test','https://ssc.gov.in/api/attachment/uploads/masterData/NoticeBoards/Notice%20of%20CHSLE%202024_05_04_24.pdf','Official core: 10 min, English 35 WPM = about 10500 key depressions/hour. SSC instructions permit correction with Backspace. Category-wise percentage-of-error cut-offs are published with results, so the simulator does not invent one universal error cut-off.'],
  ['ssc-chsl-ldc-jsa-typing-hindi',10,30,0,0,'wpm','character','unlimited',0,'none','Hindi Unicode / approved layout','{}','ssc_chsl','verified_core','SSC CHSL LDC/JSA Typing Test','https://ssc.gov.in/api/attachment/uploads/masterData/NoticeBoards/Notice%20of%20CHSLE%202024_05_04_24.pdf','Official core: 10 min, Hindi 30 WPM = about 9000 key depressions/hour. No universal category error cut-off is invented.'],
  ['railway-rrb-ntpc-typing-skill-test',10,30,0,300,'words_wpm_accuracy','word','off',0,'none','QWERTY','{"10":300}','rrb_ntpc','verified','RRB NTPC CBTST','https://www.rrbcdg.gov.in/uploads/2024/05-NTPCG/052024-CBTST_Instructions.pdf','Official: 10-minute evaluated test, at least 300 English words, 30 WPM, 5% mistake relaxation and no editing tools for correcting typed matter. If the displayed passage is completed, retyping from the beginning is allowed.'],
  ['railway-rrb-ntpc-typing-skill-test-hindi',10,25,0,250,'words_wpm_accuracy','word','off',0,'none','Mangal / Krutidev','{"10":250}','rrb_ntpc','verified','RRB NTPC CBTST','https://www.rrbcdg.gov.in/uploads/2024/06-NTPCUG/062024NTPCUG-CBTST_Instructions.pdf','Official: 10-minute evaluated test, at least 250 Hindi words, 25 WPM, 5% mistake relaxation and no editing tools for correcting typed matter.'],
  ['state-up-upsssc-junior-assistant',5,30,0,0,'wpm','character','current_prev_word',0,'none','QWERTY','{}','upsssc_ja','verified_secondary','UPSSSC Junior Assistant / Junior Clerk / Assistant Grade-III current typing cycle','Cross-checked current-cycle instructions: TypeForExam 12-07-2026 + TypingSathi 2026','Current-cycle profile: English 30 WPM, 5 min; 1 word = 5 keystrokes; first 5 mistakes ignored and each later mistake carries a 5-word penalty. Backspace is restricted to current + immediately previous word. Reports about live error highlighting conflict, so the simulator defaults to blind/no-highlight rather than inventing assistance; the candidate may change Highlight for practice. Primary UPSSSC UI instruction PDF was not retrievable, so this profile remains secondary-verified.'],
  ['state-up-upsssc-junior-assistant-hindi',5,25,0,0,'wpm','character','current_prev_word',0,'none','Mangal Inscript / Kruti Dev 010','{}','upsssc_ja','verified_secondary','UPSSSC Junior Assistant / Junior Clerk / Assistant Grade-III current typing cycle','Cross-checked current-cycle instructions: TypeForExam 12-07-2026 + TypingSathi 2026','Current-cycle profile: Hindi 25 WPM, 5 min; same 5-keystroke, mistake-grace and penalty model. Backspace current + previous word. Live-highlight reports conflict, so default is blind/no-highlight.'],
  ['state-delhi-dda-junior-secretariat-assistant',10,35,0,0,'wpm','character','unlimited',0,'current_word','QWERTY','{}','dda_jsa','verified','DDA Direct Recruitment JSA typing instructions','https://dda.gov.in/sites/default/files/latest_jobs/jsa_eng_typing03092024.pdf','Official DDA UI: unrestricted typing; current word/text indication; correct text green, incorrect text red; correction using Backspace/Arrow keys is permitted. English 35 WPM, 10 min.'],
  ['state-delhi-dda-junior-secretariat-assistant-hindi',10,30,0,0,'wpm','character','unlimited',0,'current_word','Hindi','{}','dda_jsa','verified','DDA Direct Recruitment JSA typing instructions','https://dda.gov.in/direct-recruitment-2023-hindi-typing-instructions-post-junior-secretariat-assistant-post-code-09','Hindi 30 WPM, 10 min; DDA unrestricted-mode evaluation/UI profile.'],
  ['delhi-police-hcm-typing',10,30,0,0,'wpm','character','unlimited',0,'none','QWERTY','{"10":400}','delhi_police_hcm','verified_mixed','SSC Delhi Police Head Constable (Ministerial) Examination 2025','https://ssc.gov.in/api/attachment/uploads/masterData/NoticeBoards/Notice_of_DPHCM_2025.pdf?mode=light','Official core: 10 min, English minimum 30 WPM; printed passage minimum 400 words/2000 strokes; one word per mistake. Backspace is enabled in current practice/test reports; live highlighting is not asserted by SSC notice.'],
  ['delhi-police-hcm-typing-hindi',10,25,0,0,'wpm','character','unlimited',0,'none','Hindi','{"10":350}','delhi_police_hcm','verified_mixed','SSC Delhi Police Head Constable (Ministerial) Examination 2025','https://ssc.gov.in/api/attachment/uploads/masterData/NoticeBoards/Notice_of_DPHCM_2025.pdf?mode=light','Official core: 10 min, Hindi minimum 25 WPM; printed passage minimum 350 words/1750 strokes; one word per mistake.'],
  ['state-delhi-police-head-constable-ministerial',10,30,0,0,'wpm','character','unlimited',0,'none','QWERTY','{"10":400}','delhi_police_hcm','verified_mixed','SSC Delhi Police Head Constable (Ministerial) Examination 2025','https://ssc.gov.in/api/attachment/uploads/masterData/NoticeBoards/Notice_of_DPHCM_2025.pdf?mode=light','Same verified English core as the dedicated Delhi Police HCM folder.'],
  ['state-delhi-police-head-constable-ministerial-hindi',10,25,0,0,'wpm','character','unlimited',0,'none','Hindi','{"10":350}','delhi_police_hcm','verified_mixed','SSC Delhi Police Head Constable (Ministerial) Examination 2025','https://ssc.gov.in/api/attachment/uploads/masterData/NoticeBoards/Notice_of_DPHCM_2025.pdf?mode=light','Same verified Hindi core as the dedicated Delhi Police HCM folder.']
 ];
 ...
 const missingVariants=[
  ...
  ['Railway - RRB NTPC Typing Skill Test - Hindi','railway-rrb-ntpc-typing-skill-test-hindi','Hindi','Mangal / Krutidev',10,25,0],
  ['UP - UPSSSC Junior Assistant - Hindi','state-up-upsssc-junior-assistant-hindi','Hindi','Mangal Inscript / Kruti Dev 010',5,25,0],
  ['Delhi - DDA Junior Secretariat Assistant - Hindi','state-delhi-dda-junior-secretariat-assistant-hindi','Hindi','Hindi',10,30,0],
  ['Delhi Police - Head Constable (Ministerial) Typing - Hindi','delhi-police-hcm-typing-hindi','Hindi','Hindi',10,25,0],
  ['Delhi - Delhi Police Head Constable (Ministerial) - Hindi','state-delhi-police-head-constable-ministerial-hindi','Hindi','Hindi',10,25,0]
 ];
 const update=db.prepare(`UPDATE exams SET duration=?,required_wpm=?,required_accuracy=?,min_words=?,min_chars=0,qualification_method=?,default_result_count_mode=?,backspace_allowed=?,backspace_mode=?,backspace_limit=?,highlight_mode=?,highlight_user_change_allowed=1,layout=?,duration_word_map=?,evaluation_rule=?,official_rule_status=?,official_rule_cycle=?,official_rule_source=?,official_rule_note=? WHERE slug=?`);
 ...
 db.transaction(()=>{
  for(const v of missingVariants)ensureVariant.run(v[0],v[1],v[2],v[3],v[4],v[5],v[6],1,'full','Language variant for exact exam profile; see rule provenance.',1,0,0,30,4,'none');
  for(const [name,slug] of STATE_EXAM_DIRECTORY){markNeeds.run(broadNote,slug);markNeeds.run(broadNote,slug+'-hindi')}
  for(const slug of ['ssc-selection-post-typing','up-police-ministerial-typing','central-dsssb-typing-skill-posts','railway-rrb-ministerial-stenographer-typing-posts','ssc-english','hindi-unicode','hindi-remington','krutidev-hindi','up-govt','custom-english'])markNeeds.run(broadNote,slug);
  for(const r of verified){const [slug,duration,wpm,acc,minWords,method,countMode,bsMode,bsLimit,hl,layout,map,evalRule,status,cycle,source,note]=r;const cur=db.prepare('SELECT official_rule_status FROM exams WHERE slug=?').get(slug);if(String(cur?.official_rule_status||'')==='owner_configured_manual')continue;update.run(duration,wpm,acc,minWords,method,countMode,bsMode==='off'?0:1,bsMode,bsLimit,hl,layout,map,evalRule,status,cycle,source,note,slug);updatePass.run(countMode,hl,slug)}
...
app.post('/api/auth/google',authRateLimit,async(req,res)=>{try{
 ...
 res.json({user:x,token:jwt.sign({id:x.id},SECRET,{expiresIn:'7d'}),new_account:!u.created_at});
...
function attemptMatterContentWithInputLimit(content,exam,minutes,inputLimit){
 const n=Math.max(0,Math.min(5000,Math.floor(Number(inputLimit)||0)));
 if(!n)return attemptMatterContent(content,exam,minutes);
 const raw=String(content||'').trim(),words=Array.from(raw.matchAll(/\S+/g));
 let base=raw;
 if(words.length>n){const last=words[n-1],end=Number(last.index||0)+last[0].length;base=raw.slice(0,end).trimEnd()}
 if(String(exam?.evaluation_rule||'')==='rrb_ntpc'&&base)return [base,base,base].join(' ');
 return base;
}
...
app.post('/api/results',auth,(req,res)=>{
 ...
 }else if(e&&evalRule==='upsssc_ja'){
  const rawMistakes=officialWords.full+(officialWords.half/2),freeMistakes=Math.min(5,rawMistakes),chargeableMistakes=Math.max(0,rawMistakes-5),grossStandardWords=typedChars/5,penaltyWords=chargeableMistakes*5;
  finalGross=grossStandardWords/scheduledMinutes;finalNet=Math.max(0,(grossStandardWords-penaltyWords)/scheduledMinutes);finalAccuracy=typedWords?Math.max(0,(typedWords-rawMistakes)/typedWords*100):0;
  evaluationDetails={rule:'UPSSSC Junior Assistant',gross_keystrokes:typedChars,standard_words:grossStandardWords,full_mistakes:officialWords.full,half_mistakes:officialWords.half,raw_mistakes:rawMistakes,free_mistakes:freeMistakes,chargeable_mistakes:chargeableMistakes,penalty_words:penaltyWords,formula:'(gross keystrokes/5 - max(0,mistakes-5)*5) / time'};
...
app.post('/api/admin/passages',auth,admin,(req,res,next)=>{
 const b=req.body||{},examId=Number(b.exam_id||b.exam_code||b.exam_folder_id)||0;
 if(!examId)return next();
 const exam=db.prepare('SELECT * FROM exams WHERE id=?').get(examId);
 if(!exam)return next();
 const selectedMinutes=Math.max(1,Number(b.duration_override)||Number(exam.duration)||10);
...
// Per-practice-passage payment controls. Global Payment System OFF always bypasses these locks.
for(const [c,t] of Object.entries({practice_paid_enabled:'INTEGER NOT NULL DEFAULT 0',practice_fee_amount:'REAL NOT NULL DEFAULT 0',practice_validity_days:'INTEGER NOT NULL DEFAULT 30',practice_daily_demo_limit:'INTEGER NOT NULL DEFAULT 4'})){const cs=db.prepare('PRAGMA table_info(passages)').all().map(x=>x.name);if(!cs.includes(c))db.exec(`ALTER TABLE passages ADD COLUMN ${c} ${t}`)}
...
app.post('/api/razorpay/verify',auth,async(req,res)=>{try{const b=req.body||{},orderId=String(b.razorpay_order_id||''),paymentId=String(b.razorpay_payment_id||''),sig=String(b.razorpay_signature||''),kind=String(b.kind||''),ref=String(b.ref_id||'');if(!orderId||!paymentId||!sig)return res.status(400).json({error:'Incomplete payment response'});const expected=crypto.createHmac('sha256',String(process.env.RAZORPAY_KEY_SECRET||'')).update(orderId+'|'+paymentId).digest('hex');const a=Buffer.from(expected),z=Buffer.from(sig);if(a.length!==z.length||!crypto.timingSafeEqual(a,z))return res.status(400).json({error:'Payment signature verification failed'});const t=paymentTarget(req.user,{kind,ref_id:ref});let order=await razorpayApi('/orders/'+encodeURIComponent(orderId));let payment=await razorpayApi('/payments/'+encodeURIComponent(paymentId));
 ...
 if(Number(order.amount)!==expectedAmount||Number(payment.amount)!==expectedAmount)return res.status(400).json({error:'Payment amount mismatch'});
...
app.post('/api/overall-payment',auth,(req,res)=>{
 if(!paymentSystemEnabled())return res.status(403).json({error:'Payment System is OFF — site is FREE'});
