...
const galleryStorage=multer.diskStorage({destination:(req,file,cb)=>cb(null,UPLOAD_DIR),filename:(req,file,cb)=>{const ext=path.extname(file.originalname||'').toLowerCase().replace(/[^.a-z0-9]/g,'').slice(0,8);cb(null,Date.now()+'-'+crypto.randomBytes(5).toString('hex')+ext)}});
const galleryUpload=multer({storage:galleryStorage,limits:{fileSize:100*1024*1024},fileFilter:(req,file,cb)=>{const m=String(file.mimetype||'');cb(m.startsWith('image/')||m.startsWith('video/')?null:new Error('Only image/video files allowed'),m.startsWith('image/')||m.startsWith('video/'))}});
app.use('/gallery-media',express.static(UPLOAD_DIR,{maxAge:'1d'}));
...
const aboutStorage=multer.diskStorage({destination:(req,file,cb)=>cb(null,ABOUT_UPLOAD_DIR),filename:(req,file,cb)=>{const ext=path.extname(file.originalname||'').toLowerCase().replace(/[^.a-z0-9]/g,'').slice(0,8)||'.png';cb(null,Date.now()+'-'+crypto.randomBytes(5).toString('hex')+ext)}});
...
function scheduleRemoteSqliteMirror(){
  ...
  // Batch frequent writes into one compressed remote snapshot to prevent bandwidth spikes.
...
db.exec(`CREATE TABLE IF NOT EXISTS visitor_analytics(id INTEGER PRIMARY KEY AUTOINCREMENT,visitor_id TEXT NOT NULL,page TEXT DEFAULT 'home',device TEXT DEFAULT 'Desktop',source TEXT DEFAULT 'Direct',created_at TEXT DEFAULT CURRENT_TIMESTAMP); CREATE INDEX IF NOT EXISTS idx_visitor_analytics_created ON visitor_analytics(created_at); CREATE INDEX IF NOT EXISTS idx_visitor_analytics_vid ON visitor_analytics(visitor_id);`);
db.exec(`CREATE TABLE IF NOT EXISTS referral_codes(user_id INTEGER PRIMARY KEY,code TEXT NOT NULL UNIQUE,created_at TEXT DEFAULT CURRENT_TIMESTAMP,FOREIGN KEY(user_id) REFERENCES users(id)); CREATE TABLE IF NOT EXISTS referrals(id INTEGER PRIMARY KEY AUTOINCREMENT,referrer_id INTEGER NOT NULL,referred_user_id INTEGER NOT NULL UNIQUE,ref_code TEXT NOT NULL,created_at TEXT DEFAULT CURRENT_TIMESTAMP,FOREIGN KEY(referrer_id) REFERENCES users(id),FOREIGN KEY(referred_user_id) REFERENCES users(id)); CREATE INDEX IF NOT EXISTS idx_referrals_referrer ON referrals(referrer_id);`);
...
db.transaction(()=>{
  let owner=db.prepare("SELECT * FROM users WHERE lower(email)=? AND is_owner=1 ORDER BY id LIMIT 1").get(MASTER_OWNER_EMAIL)
    ...
    || db.prepare("SELECT * FROM users WHERE is_owner=1 ORDER BY id LIMIT 1").get();
  if(!owner){
    ...
    else {const id=db.prepare('INSERT INTO users(name,email,password,role,is_owner,plan,owner_uid) VALUES(?,?,?,?,1,?,?)').run("Shivjee's Owner",MASTER_OWNER_EMAIL,bcrypt.hashSync(adminPass,10),'admin','Master Owner','MASTER-OWNER-001').lastInsertRowid;owner=db.prepare('SELECT * FROM users WHERE id=?').get(id)}
  }
  ...
  if(MASTER_OWNER_PHONE){const phoneConflict=db.prepare('SELECT id FROM users WHERE phone=? AND id<>?').get(MASTER_OWNER_PHONE,owner.id);if(phoneConflict)db.prepare('UPDATE users SET phone=NULL,phone_verified=0 WHERE id=?').run(phoneConflict.id)}
  db.prepare("UPDATE users SET email=?,phone=?,phone_verified=1,is_owner=1,role='admin',active=1,plan='Master Owner',owner_uid='MASTER-OWNER-001' WHERE id=?").run(MASTER_OWNER_EMAIL,MASTER_OWNER_PHONE,owner.id);
...
const VERIFIED_TYPING_EXAM_DIRECTORY=[
 ...
 ['SSC - CGL DEST / Data Entry Speed Test','ssc-cgl-dest','English','QWERTY',15,0,0],
 ['SSC - Selection Post Typing Test','ssc-selection-post-typing','English','QWERTY',10,0,0],
 ['SSC - Stenographer Computer Transcription','ssc-stenographer-transcription','English','QWERTY',10,0,0],
 ['Railway - RRB NTPC Typing Skill Test','railway-rrb-ntpc-typing-skill-test','English','QWERTY',10,30,0],
 ['Railway - Ministerial / Typing Posts','railway-ministerial-typing-posts','English','QWERTY',10,0,0],
 ['Delhi - Delhi Police Head Constable (Ministerial)','delhi-police-hcm-typing','English','QWERTY',10,30,0],
 ['Delhi - DSSSB Junior Assistant / LDC Typing','delhi-dsssb-ja-ldc-typing','English','QWERTY',10,0,0],
 ['Delhi - High Court JJA Typing','delhi-high-court-jja-typing','English','QWERTY',10,0,0],
 ['UP - UP Police Computer Operator','up-police-computer-operator-typing','English','QWERTY',15,0,0],
 ['UP - UPSSSC Junior Assistant Typing','up-upsssc-junior-assistant-typing','Hindi','Unicode / Mangal',10,0,0],
 ['UP - Allahabad High Court Junior Assistant Typing','up-allahabad-hc-ja-typing','English','QWERTY',10,0,0],
 ['Bihar - BSSC / Clerk Typing Posts','bihar-bssc-clerk-typing','Hindi','Unicode / Mangal',10,0,0],
 ['Bihar - Civil Court Clerk Typing','bihar-civil-court-clerk-typing','English','QWERTY',10,0,0],
 ['Jharkhand - JSSC Clerk Typing Posts','jharkhand-jssc-clerk-typing','Hindi','Unicode / Mangal',10,0,0],
 ['Rajasthan - RSSB LDC / Junior Assistant Typing','rajasthan-rssb-ldc-ja-typing','Hindi','Unicode / Mangal',10,0,0],
 ['Rajasthan - High Court JJA / Clerk Typing','rajasthan-high-court-jja-clerk-typing','English','QWERTY',10,0,0],
 ['Madhya Pradesh - CPCT Typing','mp-cpct-typing','Hindi','Unicode / Mangal',15,0,0],
 ['Madhya Pradesh - High Court Assistant / Clerk Typing','mp-high-court-clerk-typing','English','QWERTY',10,0,0],
 ['Chhattisgarh - High Court Assistant Typing','chhattisgarh-high-court-assistant-typing','English','QWERTY',10,0,0],
 ['Haryana - HSSC Clerk / Typing Posts','haryana-hssc-clerk-typing','English','QWERTY',10,0,0],
 ['Punjab - PSSSB Clerk / IT Clerk Typing','punjab-psssb-clerk-typing','English','QWERTY',10,0,0],
 ['Himachal Pradesh - JOA (IT) Typing','hp-joa-it-typing','English','QWERTY',10,0,0],
 ['Uttarakhand - UKSSSC Junior Assistant / DEO Typing','uttarakhand-uksssc-ja-deo-typing','Hindi','Unicode / Mangal',10,0,0],
 ['Jammu & Kashmir - JKSSB Junior Assistant Typing','jk-jkssb-junior-assistant-typing','English','QWERTY',10,0,0],
 ['Gujarat - High Court / Clerk Typing Posts','gujarat-high-court-clerk-typing','English','QWERTY',10,0,0],
 ['Maharashtra - Clerk-Typist Typing','maharashtra-clerk-typist-typing','English','QWERTY',10,0,0],
 ['Goa - Government Clerk / Typing Posts','goa-clerk-typing','English','QWERTY',10,0,0],
 ['West Bengal - Clerk / Typist Posts','west-bengal-clerk-typist','English','QWERTY',10,0,0],
 ['Odisha - Junior Assistant / Typing Posts','odisha-junior-assistant-typing','English','QWERTY',10,0,0],
 ['Assam - Junior Assistant / Typing Posts','assam-junior-assistant-typing','English','QWERTY',10,0,0],
 ['Sikkim - Clerk / Typing Posts','sikkim-clerk-typing','English','QWERTY',10,0,0],
 ['Arunachal Pradesh - Clerk / Typing Posts','arunachal-clerk-typing','English','QWERTY',10,0,0],
 ['Nagaland - Clerk / Typing Posts','nagaland-clerk-typing','English','QWERTY',10,0,0],
 ['Manipur - Clerk / Typing Posts','manipur-clerk-typing','English','QWERTY',10,0,0],
 ['Mizoram - Clerk / Typing Posts','mizoram-clerk-typing','English','QWERTY',10,0,0],
 ['Tripura - Clerk / Typing Posts','tripura-clerk-typing','English','QWERTY',10,0,0],
 ['Meghalaya - Clerk / Typing Posts','meghalaya-clerk-typing','English','QWERTY',10,0,0],
 ['Andhra Pradesh - High Court Assistant / Typing Posts','andhra-pradesh-high-court-typing','English','QWERTY',10,0,0],
 ['Telangana - High Court Junior Assistant Typing','telangana-high-court-ja-typing','English','QWERTY',10,0,0],
 ['Karnataka - High Court / Typist Posts','karnataka-high-court-typist','English','QWERTY',10,0,0],
 ['Kerala - High Court Assistant / Typist Posts','kerala-high-court-typist','English','QWERTY',10,0,0],
 ['Tamil Nadu - High Court / Typist Posts','tamil-nadu-high-court-typist','English','QWERTY',10,0,0],
 ['Puducherry - Clerk / Typing Posts','puducherry-clerk-typing','English','QWERTY',10,0,0],
 ['Chandigarh - Administration Clerk / Junior Assistant Typing','chandigarh-clerk-ja-typing','English','QWERTY',10,0,0],
 ['Ladakh - Junior Assistant / Typing Posts','ladakh-junior-assistant-typing','English','QWERTY',10,0,0]
];
...
// Safety correction 2026-09-14: legacy generated exam folders had guessed Current Character/Word highlighting.
...
db.prepare("UPDATE passages SET highlight_mode='none' WHERE exam_id IN (SELECT id FROM exams WHERE slug IN ('upp-co-english','upp-co-hindi','ssc-chsl-ldc-jsa-typing','ssc-selection-post-typing','delhi-police-hcm-typing','up-police-ministerial-typing','railway-rrb-ntpc-typing-skill-test'))").run();
...
app.post('/api/auth/admin-register-start',authRateLimit,async(req,res)=>{try{let{name,father_name,dob,phone,email,password}=req.body||{};name=String(name||'').trim().replace(/\s+/g,' ');father_name=String(father_name||'').trim().replace(/\s+/g,' ');dob=String(dob||'').trim();phone=normalizePhone(phone);email=String(email||'').trim().toLowerCase();password=String(password||'');if(name.length<2||name.length>80)return res.status(400).json({error:'Enter a valid full name'});if(father_name.length<2||father_name.length>80)return res.status(400).json({error:"Enter father's name"});if(!/^\d{4}-\d{2}-\d{2}$/.test(dob)||new Date(dob+'T00:00:00')>new Date())return res.status(400).json({error:'Enter a valid date of birth'});if(!phone)return res.status(400).json({error:'Enter a valid 10-digit Indian mobile number'});if(!emailOk(email)||email.length>160)return res.status(400).json({error:'Enter a valid email address'});if(password.length<10||password.length>200)return res.status(400).json({error:'Admin password must be at least 10 characters'});
 ...
 const purpose=ownerSetup?'owner_setup':'admin_signup';const latest=db.prepare('SELECT created_at FROM otp_codes WHERE phone=? AND purpose=? ORDER BY id DESC LIMIT 1').get(phone,purpose);if(latest&&Date.now()-new Date(latest.created_at+'Z').getTime()<30000)return res.status(429).json({error:'Please wait 30 seconds before requesting another signup OTP'});const code=String(crypto.randomInt(100000,1000000)),expires=new Date(Date.now()+5*60*1000).toISOString();db.prepare('UPDATE otp_codes SET used=1 WHERE phone=? AND purpose=? AND used=0').run(phone,purpose);db.prepare('INSERT INTO otp_codes(phone,purpose,code_hash,expires_at) VALUES(?,?,?,?)').run(phone,purpose,otpHash(phone,purpose,code),expires);const delivered=await deliverOtp(phone,code,email,purpose);const challenge=jwt.sign({type:purpose,name,father_name,dob,phone,email,password_hash:bcrypt.hashSync(password,10),existing_owner_id:ownerSetup?existing.id:null},SECRET,{expiresIn:'10m'});res.json({challenge,phone_masked:maskPhone(phone),email_masked:delivered.email_masked||maskEmail(email),owner_setup:ownerSetup,...(delivered.dev?{dev_otp:code}:{})});}catch(e){res.status(503).json({error:e.message||'Could not send signup OTP'})}});
...
function referralCodeFor(userId){let r=db.prepare('SELECT code FROM referral_codes WHERE user_id=?').get(userId);if(r)return r.code;let code='JP'+Number(userId).toString(36).toUpperCase().padStart(4,'0');while(db.prepare('SELECT 1 FROM referral_codes WHERE code=?').get(code))code='JP'+Number(userId).toString(36).toUpperCase()+crypto.randomInt(10,99);db.prepare('INSERT INTO referral_codes(user_id,code) VALUES(?,?)').run(userId,code);return code;}
app.post('/api/auth/register-start',authRateLimit,async(req,res)=>{try{if(setting('allow_registration')!=='1')return res.status(403).json({error:'New registration is currently disabled'});let{name,father_name,dob,target_exam,state,district,phone,email,password,ref}=req.body||{};ref=String(ref||'').trim().toUpperCase().slice(0,40);name=String(name||'').trim().replace(/\s+/g,' ');father_name=String(father_name||'').trim().replace(/\s+/g,' ');dob=String(dob||'').trim();target_exam=String(target_exam||'').trim().slice(0,120);state=String(state||'').trim().slice(0,80);district=String(district||'').trim().slice(0,80);phone=normalizePhone(phone);email=String(email||'').trim().toLowerCase();password=String(password||'');if(name.length<2||name.length>80)return res.status(400).json({error:'Enter a valid full name'});if(father_name.length<2||father_name.length>80)return res.status(400).json({error:"Enter father's name"});if(!/^\d{4}-\d{2}-\d{2}$/.test(dob)||new Date(dob+'T00:00:00')>new Date())return res.status(400).json({error:'Enter a valid date of birth'});if(!target_exam)return res.status(400).json({error:'Select the exam you are preparing for'});if(!state)return res.status(400).json({error:'Enter your state'});if(!district)return res.status(400).json({error:'Enter your district'});if(!phone)return res.status(400).json({error:'Enter a valid 10-digit Indian mobile number'});if(!emailOk(email)||email.length>160)return res.status(400).json({error:'Enter a valid email address'});if(password.length<8||password.length>200)return res.status(400).json({error:'Password must be 8–200 characters'});if(db.prepare('SELECT 1 FROM users WHERE lower(email)=?').get(email))return res.status(400).json({error:'Email already registered'});if(db.prepare('SELECT 1 FROM users WHERE phone=?').get(phone))return res.status(400).json({error:'Mobile number already registered'});const purpose='candidate_signup';const latest=db.prepare('SELECT created_at FROM otp_codes WHERE phone=? AND purpose=? ORDER BY id DESC LIMIT 1').get(phone,purpose);if(latest&&Date.now()-new Date(latest.created_at+'Z').getTime()<30000)return res.status(429).json({error:'Please wait 30 seconds before requesting another signup OTP'});const code=String(crypto.randomInt(100000,1000000)),expires=new Date(Date.now()+5*60*1000).toISOString();db.prepare('UPDATE otp_codes SET used=1 WHERE phone=? AND purpose=? AND used=0').run(phone,purpose);db.prepare('INSERT INTO otp_codes(phone,purpose,code_hash,expires_at) VALUES(?,?,?,?)').run(phone,purpose,otpHash(phone,purpose,code),expires);const delivered=await deliverOtp(phone,code,email,purpose);const challenge=jwt.sign({type:purpose,name,father_name,dob,target_exam,state,district,phone,email,password_hash:bcrypt.hashSync(password,10),ref},SECRET,{expiresIn:'10m'});res.json({verification_required:true,challenge,email_masked:delivered.email_masked||maskEmail(email),...(delivered.dev?{dev_otp:code}:{})});}catch(e){res.status(503).json({error:e.message||'Could not send signup OTP'})}});
app.post('/api/auth/register-verify',authRateLimit,(req,res)=>{let c;try{c=jwt.verify(String(req.body?.challenge||''),SECRET)}catch{return res.status(401).json({error:'Signup verification expired. Start again.'})}if(c?.type!=='candidate_signup')return res.status(401).json({error:'Invalid signup verification'});const otp=String(req.body?.otp||'').trim();if(!/^\d{6}$/.test(otp))return res.status(400).json({error:'Enter the 6-digit OTP'});const row=db.prepare("SELECT * FROM otp_codes WHERE phone=? AND purpose='candidate_signup' AND used=0 ORDER BY id DESC LIMIT 1").get(c.phone);if(!row)return res.status(400).json({error:'Request a new signup OTP'});if(new Date(row.expires_at)<new Date()){db.prepare('UPDATE otp_codes SET used=1 WHERE id=?').run(row.id);return res.status(400).json({error:'OTP expired. Start signup again.'})}if(row.attempts>=5){db.prepare('UPDATE otp_codes SET used=1 WHERE id=?').run(row.id);return res.status(429).json({error:'Too many wrong OTP attempts. Start signup again.'})}if(otpHash(c.phone,'candidate_signup',otp)!==row.code_hash){db.prepare('UPDATE otp_codes SET attempts=attempts+1 WHERE id=?').run(row.id);return res.status(401).json({error:'Incorrect signup OTP'})}db.prepare('UPDATE otp_codes SET used=1 WHERE id=?').run(row.id);try{if(db.prepare('SELECT 1 FROM users WHERE lower(email)=? OR phone=?').get(c.email,c.phone))return res.status(400).json({error:'Email or mobile number already registered'});const id=db.prepare('INSERT INTO users(name,father_name,dob,target_exam,state,district,phone,email,password,phone_verified) VALUES(?,?,?,?,?,?,?,?,?,1)').run(c.name,c.father_name,c.dob,c.target_exam,c.state,c.district,c.phone,c.email,c.password_hash).lastInsertRowid;if(c.ref){const rr=db.prepare('SELECT user_id FROM referral_codes WHERE code=?').get(String(c.ref).toUpperCase());if(rr&&Number(rr.user_id)!==Number(id)){db.prepare('INSERT OR IGNORE INTO referrals(referrer_id,referred_user_id,ref_code) VALUES(?,?,?)').run(rr.user_id,id,String(c.ref).toUpperCase());}}const u=safe(db.prepare('SELECT * FROM users WHERE id=?').get(id));referralCodeFor(id);res.json({user:u,token:jwt.sign({id:u.id},SECRET,{expiresIn:'7d'})})}catch(e){res.status(400).json({error:'Could not create account'})}});
...
app.post('/api/auth/admin-login-start',authRateLimit,async(req,res)=>{try{
 ...
 try{delivered=await deliverOtp(phone,code,u.email,'admin_2fa')}catch(otpErr){
...
// Until an SMS provider is connected, the OTP is shown on the Owner screen after password verification.
app.post('/api/auth/owner-otp-start',authRateLimit,async(req,res)=>{try{
 ...
 db.prepare('INSERT INTO otp_codes(phone,purpose,code_hash,expires_at) VALUES(?,?,?,?)').run(phone,'owner_login',otpHash(phone,'owner_login',code),expires);
 let delivered={dev:true};
...
app.post('/api/auth/owner-otp-verify',authRateLimit,(req,res)=>{try{
 ...
 if(!c?.ownerOtp)return res.status(401).json({error:'Invalid Owner verification'});
 const otp=String(req.body?.otp||'').trim();if(!/^\d{6}$/.test(otp))return res.status(400).json({error:'Enter the 6-digit OTP'});
 const u=db.prepare("SELECT * FROM users WHERE id=? AND role='admin' AND is_owner=1").get(c.ownerOtp);if(!u)return res.status(404).json({error:'Master Owner account not found'});
 const phone=normalizePhone(c.phone||u.phone);if(!phone)return res.status(400).json({error:'Owner mobile is not linked'});
 // Owner OTP is bound into the signed 5-minute challenge created only after password verification.
 // This makes local/testing verification reliable even if an old SQLite OTP row is stale or missing.
 const enteredHash=otpHash(phone,'owner_login',otp);
 const challengeOk=!!c.otp_hash && enteredHash===c.otp_hash;
 const row=db.prepare("SELECT * FROM otp_codes WHERE phone=? AND purpose='owner_login' AND used=0 ORDER BY id DESC LIMIT 1").get(phone);
 let rowOk=false;
 if(row){
   if(new Date(row.expires_at)>=new Date() && Number(row.attempts||0)<5) rowOk=enteredHash===row.code_hash;
 }
 if(!challengeOk && !rowOk){if(row)db.prepare('UPDATE otp_codes SET attempts=COALESCE(attempts,0)+1 WHERE id=?').run(row.id);return res.status(401).json({error:'Incorrect Owner OTP'})}
 if(row)db.prepare('UPDATE otp_codes SET used=1 WHERE id=?').run(row.id);
 ...
 const x=safe(db.prepare('SELECT * FROM users WHERE id=?').get(u.id));res.json({user:x,token:jwt.sign({id:x.id},SECRET,{expiresIn:'7d'})});
...
app.post('/api/auth/forgot-password-start',authRateLimit,async(req,res)=>{try{
 ...
 const delivered=await deliverOtp(phone,code,u.email,'password_reset'),challenge=jwt.sign({type:'password_reset',uid:u.id,phone},SECRET,{expiresIn:'10m'});
...
app.post('/api/auth/google',authRateLimit,async(req,res)=>{try{
 ...
 const credential=String(req.body?.credential||'').trim();if(!credential)return res.status(400).json({error:'Google sign-in credential missing'});
 ...
 if(u){
   ...
   if(!u.google_sub)db.prepare('UPDATE users SET google_sub=? WHERE id=?').run(sub,u.id);
 }else{
   ...
   const id=db.prepare("INSERT INTO users(name,email,password,role,active,plan,google_sub) VALUES(?,?,?,'student',1,'Free',?)").run(name||'Candidate',email,randomPassword,sub).lastInsertRowid;
...
app.get('/api/referral/me',auth,(req,res)=>{const code=referralCodeFor(req.user.id);const total=db.prepare('SELECT COUNT(*) n FROM referrals WHERE referrer_id=?').get(req.user.id).n;const paid=db.prepare("SELECT COUNT(DISTINCT r.referred_user_id) n FROM referrals r JOIN payment_requests p ON p.user_id=r.referred_user_id AND p.status='approved' WHERE r.referrer_id=?").get(req.user.id).n;const recent=db.prepare('SELECT u.name,r.created_at FROM referrals r JOIN users u ON u.id=r.referred_user_id WHERE r.referrer_id=? ORDER BY r.id DESC LIMIT 10').all(req.user.id);res.json({code,link:'https://jptyping.in/?ref='+encodeURIComponent(code),signups:total,paid,recent})});
app.get('/api/admin/referrals',auth,admin,(req,res)=>{const summary=db.prepare("SELECT COUNT(*) referrals, COUNT(DISTINCT referrer_id) referrers FROM referrals").get();const paid=db.prepare("SELECT COUNT(DISTINCT r.referred_user_id) n FROM referrals r JOIN payment_requests p ON p.user_id=r.referred_user_id AND p.status='approved'").get().n;const rows=db.prepare("SELECT rc.code,u.id user_id,u.name,u.email,COUNT(r.id) signups,COUNT(DISTINCT CASE WHEN p.status='approved' THEN r.referred_user_id END) paid FROM referral_codes rc JOIN users u ON u.id=rc.user_id LEFT JOIN referrals r ON r.referrer_id=u.id LEFT JOIN payment_requests p ON p.user_id=r.referred_user_id GROUP BY rc.code,u.id,u.name,u.email HAVING COUNT(r.id)>0 ORDER BY signups DESC,u.id DESC LIMIT 200").all();const recent=db.prepare("SELECT r.created_at,r.ref_code,a.name referrer,b.name referred,b.email referred_email FROM referrals r JOIN users a ON a.id=r.referrer_id JOIN users b ON b.id=r.referred_user_id ORDER BY r.id DESC LIMIT 100").all();res.json({referrals:summary.referrals,referrers:summary.referrers,paid,rows,recent})});
app.get('/api/me',auth,(req,res)=>res.json(req.user));
// Candidate self-service mobile capture (used after Google Sign-In when Google does not provide a phone number).
app.patch('/api/me/mobile',auth,(req,res)=>{try{if(String(req.user.role||'').toLowerCase()==='admin'||Number(req.user.is_owner)===1)return res.status(403).json({error:'Owner/Admin mobile is managed separately'});const phone=normalizePhone(req.body?.phone);if(!phone)return res.status(400).json({error:'Enter a valid 10-digit Indian mobile number'});const used=db.prepare('SELECT id FROM users WHERE phone=? AND id<>? LIMIT 1').get(phone,req.user.id);if(used)return res.status(409).json({error:'This mobile number is already linked with another account'});db.prepare('UPDATE users SET phone=? WHERE id=?').run(phone,req.user.id);const x=safe(db.prepare('SELECT * FROM users WHERE id=?').get(req.user.id));res.json({ok:true,user:x});}catch(e){res.status(500).json({error:'Could not save mobile number'})}});
// One-time profile setup for a brand-new Candidate created through Google Sign-In.
app.patch('/api/me/google-first-profile',auth,(req,res)=>{try{const current=db.prepare('SELECT * FROM users WHERE id=?').get(req.user.id);if(!current||String(current.role||'').toLowerCase()==='admin'||Number(current.is_owner)===1)return res.status(403).json({error:'Candidate account required'});if(!String(current.google_sub||'').trim())return res.status(403).json({error:'This setup is only for Google Sign-In candidates'});if(Number(current.google_profile_completed)===1)return res.status(409).json({error:'Google profile setup is already complete'});const name=String(req.body?.name||'').trim().replace(/\s+/g,' ').slice(0,80),phone=normalizePhone(req.body?.phone),password=String(req.body?.password||'');if(name.length<2)return res.status(400).json({error:'Enter your full name'});if(!phone)return res.status(400).json({error:'Enter a valid 10-digit Indian mobile number'});if(password.length<8||password.length>200)return res.status(400).json({error:'Password must be 8–200 characters'});const used=db.prepare('SELECT id FROM users WHERE phone=? AND id<>? LIMIT 1').get(phone,current.id);if(used)return res.status(409).json({error:'This mobile number is already linked with another account'});db.prepare('UPDATE users SET name=?,phone=?,password=?,phone_verified=0,google_profile_completed=1 WHERE id=?').run(name,phone,bcrypt.hashSync(password,10),current.id);const x=safe(db.prepare('SELECT * FROM users WHERE id=?').get(current.id));res.json({ok:true,user:x,login_id:phone});}catch(e){res.status(500).json({error:'Could not save Google account details'})}});
...
app.get('/api/exams/:id',auth,(req,res)=>{res.set('Cache-Control','no-store');const e=db.prepare('SELECT * FROM exams WHERE id=? AND active=1').get(req.params.id);if(!e)return res.status(404).json({error:'Exam not found'});const cfg=examFolderConfig(e),state=examAccessState(req.user,cfg);if(state.blocked)return res.status(403).json({error:'This exam sub-folder is blocked by Owner',code:'OWNER_BLOCKED',reason:state.block_reason});if(cfg.paid_enabled&&!state.can_start)return res.status(402).json({error:'Payment required for this exam sub-folder',code:'EXAM_PAYMENT_REQUIRED'});const p=db.prepare("SELECT * FROM passages WHERE active=1 AND exam_id=? ORDER BY id ASC LIMIT 100").all(e.id);res.json({...e,fee_amount:cfg.fee_amount,validity_days:cfg.validity_days,daily_demo_limit:cfg.daily_demo_limit,paid_enabled:cfg.paid_enabled,passages:p})});
app.get('/api/passages',(req,res)=>{let q='SELECT * FROM passages WHERE active=1',a=[];if(req.query.exam_id){const eid=Number(req.query.exam_id),raw=db.prepare('SELECT * FROM exams WHERE id=? AND active=1').get(eid);if(raw&&paymentSystemEnabled()&&Number(examFolderConfig(raw).paid_enabled)){const h=req.headers.authorization||'';if(!h.startsWith('Bearer '))return res.status(401).json({error:'Login required',code:'LOGIN_REQUIRED'});let u;try{const t=jwt.verify(h.slice(7),SECRET);u=db.prepare('SELECT id,name,email,role,active,plan,valid_until,phone,father_name,dob,target_exam,phone_verified,is_owner,owner_uid FROM users WHERE id=?').get(t.id)}catch(e){return res.status(401).json({error:'Session expired'})}if(!u)return res.status(401).json({error:'Account not found'});const st=examAccessState(u,examFolderConfig(raw));if(!st.can_start)return res.status(402).json({error:'Payment required for this exam sub-folder',code:'EXAM_PAYMENT_REQUIRED'})}q+=' AND exam_id=?';a.push(eid)}else if(req.query.include_all!=='1'){q+=' AND exam_id IS NULL'}if(req.query.language){q+=' AND language=?';a.push(req.query.language)}if(req.query.layout){q+=' AND layout=?';a.push(req.query.layout)}if(req.query.difficulty){q+=' AND difficulty=?';a.push(req.query.difficulty)}res.set('Cache-Control','no-store');res.json(db.prepare(q+' ORDER BY id DESC').all(...a))});
...
function examFolderConfig(exam){
 if(!paymentSystemEnabled())return {...exam,paid_enabled:0,fee_amount:Number(exam?.fee_amount||0),validity_days:Math.max(1,Number(exam?.validity_days)||30),daily_demo_limit:Math.max(0,Number(exam?.daily_demo_limit)||0)};
...
function examAccessState(user,exam){
 ...
 if(!paymentSystemEnabled())return {has_access:true,access:{access_type:'payment_off'},demo_used:0,demo_remaining:Number.MAX_SAFE_INTEGER,bonus_remaining:Number.MAX_SAFE_INTEGER,can_start:true,source:'payment_off'};
...
app.post('/api/payment-requests',auth,(req,res)=>{if(!paymentSystemEnabled())return res.status(503).json({error:'Payment system is currently OFF by Owner',code:'PAYMENT_SYSTEM_OFF'});const b=req.body||{},eid=Number(b.exam_id),rawExam=db.prepare('SELECT * FROM exams WHERE id=? AND active=1').get(eid),exam=rawExam?examFolderConfig(rawExam):null;if(!exam||!exam.paid_enabled)return res.status(400).json({error:'Paid exam folder not found'});if(String(req.user?.role||'').toLowerCase()==='admin')return res.status(403).json({error:'Paid access is for candidates only'});const currentState=examAccessState(req.user,exam);if(currentState.blocked)return res.status(403).json({error:'This exam folder is blocked by Owner: '+(currentState.block_reason||'Access denied'),code:'OWNER_BLOCKED'});const txn=String(b.txn_ref||'').trim().slice(0,120),method=String(b.method||'manual').slice(0,30);if(!txn)return res.status(400).json({error:'Transaction Number required'});const dup=db.prepare("SELECT id,status FROM payment_requests WHERE user_id=? AND exam_id=? AND txn_ref=? AND status IN ('pending','approved')").get(req.user.id,eid,txn);if(dup)return res.json({id:dup.id,status:dup.status,duplicate:true,auto_enrolled:dup.status==='approved'});const amount=Math.max(0,Number(exam.fee_amount)||0);const id=db.prepare("INSERT INTO payment_requests(user_id,exam_id,amount,method,txn_ref,notes,status) VALUES(?,?,?,?,?,?,'pending')").run(req.user.id,eid,amount,method,txn,'Awaiting verified payment / Owner approval').lastInsertRowid;audit(req,'PAYMENT_REQUEST','payment',id,`user ${req.user.id} exam ${eid} amount ${amount}`);res.json({id,status:'pending',auto_enrolled:false,amount})});
...
app.get('/api/learning/matters/:key',auth,(req,res)=>{const key=String(req.params.key||'').slice(0,80);const gate=learningAccessState(req.user.id,key,req.user);if(!gate.allowed)return res.status(402).json({error:'Learning demo finished. Payment required.',code:'LEARNING_PAYMENT_REQUIRED',course_key:key,plan:gate.plan});res.json(db.prepare('SELECT id,course_key,title,level,matter_type,content,sort_order FROM learning_matters WHERE course_key=? AND active=1 ORDER BY sort_order,id').all(key))});
app.get('/api/admin/learning-matters',auth,admin,(req,res)=>res.json(db.prepare('SELECT * FROM learning_matters ORDER BY course_key,sort_order,id').all()));
...
app.get('/api/learning/access/:key',auth,(req,res)=>{res.set('Cache-Control','no-store');res.json(learningAccessState(req.user.id,String(req.params.key),req.user))});
...
app.get('/api/certificates/my',auth,(req,res)=>{const reg=ensureRegistrationNo(req.user.id);res.json(db.prepare("SELECT id,course_key,course_name,certificate_id,status,final_score,completion_date,approved_at,signature_url,download_count,last_download_at,created_at,payment_required,certificate_fee,payment_status,payment_txn,payment_method,paid_at,auth_token,auth_fingerprint FROM certificates WHERE user_id=? ORDER BY id DESC").all(req.user.id).map(x=>{const reqd=Number(x.payment_required)===1;const paid=reqd&&String(x.payment_status||'').toLowerCase()==='paid'&&!!(x.payment_txn||x.paid_at);const waived=!reqd&&String(x.payment_status||'').toLowerCase()==='waived'&&String(x.payment_method||'')==='owner-waived';const access=!paymentSystemEnabled()||paid||waived;return {...x,certificate_access:access?'unlocked':'locked',payment_configured:reqd||waived,registration_no:reg,verify_url:access&&x.certificate_id&&x.auth_token?'/verify-certificate?id='+encodeURIComponent(x.certificate_id)+'&proof='+encodeURIComponent(x.auth_token):null}})) });
app.post('/api/certificates/:id/download',auth,(req,res)=>{const c=db.prepare("SELECT * FROM certificates WHERE id=? AND user_id=?").get(req.params.id,req.user.id);if(!c||c.status!=='approved')return res.status(403).json({error:'Certificate is not approved'});const reqd=Number(c.payment_required)===1;const paid=reqd&&String(c.payment_status||'').toLowerCase()==='paid'&&!!(c.payment_txn||c.paid_at);const waived=!reqd&&String(c.payment_status||'').toLowerCase()==='waived'&&String(c.payment_method||'')==='owner-waived';if(paymentSystemEnabled()&&!(paid||waived))return res.status(402).json({error:reqd?'Certificate payment required before download':'Owner must set Certificate Payment or Free/Waived before download',code:'CERT_PAYMENT_REQUIRED',fee:Number(c.certificate_fee)||0});db.prepare("UPDATE certificates SET download_count=download_count+1,last_download_at=CURRENT_TIMESTAMP WHERE id=?").run(c.id);res.json({ok:true})});
app.post('/api/certificates/:id/demo-pay',auth,(req,res)=>{if(!paymentSystemEnabled())return res.status(503).json({error:'Payment system is currently OFF by Owner',code:'PAYMENT_SYSTEM_OFF'});const c=db.prepare('SELECT * FROM certificates WHERE id=? AND user_id=?').get(req.params.id,req.user.id);if(!c)return res.status(404).json({error:'Certificate not found'});if(Number(c.payment_required)!==1||Number(c.certificate_fee)<=0)return res.status(409).json({error:'Certificate payment is not configured by Owner',code:'CERT_PAYMENT_NOT_CONFIGURED'});const b=req.body||{},txn=String(b.txn_ref||'').trim().slice(0,100);if(!txn)return res.status(400).json({error:'Demo transaction number required'});db.prepare("UPDATE certificates SET payment_status='paid',payment_txn=?,payment_method=?,paid_at=CURRENT_TIMESTAMP WHERE id=?").run(txn,String(b.method||'demo').slice(0,30),c.id);res.json({status:'paid'})});
...
app.get('/api/certificates/skill-tests',auth,(req,res)=>{return res.json(db.prepare('SELECT id,certificate_id,language,difficulty,duration,required_wpm,required_accuracy,status,wpm,accuracy,passed,assigned_at,submitted_at FROM certificate_skill_tests WHERE user_id=? ORDER BY id DESC').all(req.user.id));});
app.get('/api/certificates/panel',auth,(req,res)=>{const reg=ensureRegistrationNo(req.user.id);const certs=db.prepare("SELECT id,course_key,course_name,certificate_id,status,final_score,completion_date,approved_at,signature_url,download_count,last_download_at,created_at,payment_required,certificate_fee,payment_status,payment_txn,payment_method,paid_at,auth_token,auth_fingerprint FROM certificates WHERE user_id=? ORDER BY id DESC").all(req.user.id).map(x=>{const reqd=Number(x.payment_required)===1;const paid=reqd&&String(x.payment_status||'').toLowerCase()==='paid'&&!!(x.payment_txn||x.paid_at);const waived=!reqd&&String(x.payment_status||'').toLowerCase()==='waived'&&String(x.payment_method||'')==='owner-waived';const access=!paymentSystemEnabled()||paid||waived;return {...x,certificate_access:access?'unlocked':'locked',payment_configured:reqd||waived,registration_no:reg,verify_url:access&&x.certificate_id&&x.auth_token?'/verify-certificate?id='+encodeURIComponent(x.certificate_id)+'&proof='+encodeURIComponent(x.auth_token):null}});const tests=db.prepare("SELECT id,certificate_id,language,difficulty,duration,required_wpm,required_accuracy,status,wpm,accuracy,passed,assigned_at,submitted_at FROM certificate_skill_tests WHERE user_id=? AND status<>'replaced' ORDER BY CASE status WHEN 'assigned' THEN 0 ELSE 1 END,id DESC").all(req.user.id);res.json({certificates:certs,skill_tests:tests});});
...
app.post('/api/admin/certificates/:id/approve',auth,admin,certSignUpload.single('signature'),(req,res)=>{const c=db.prepare('SELECT * FROM certificates WHERE id=?').get(req.params.id);if(!c)return res.status(404).json({error:'Certificate request not found'});const certPrefix=String(c.request_type||'course')==='skill'?'SJT-SKILL':'SJL-LEARN';const certId=c.certificate_id||(certPrefix+'-'+new Date().getFullYear()+'-'+String(c.id).padStart(6,'0')+'-'+crypto.randomBytes(2).toString('hex').toUpperCase());const sig=null;const approvedAt=new Date().toISOString();db.prepare("UPDATE certificates SET status='approved',certificate_id=?,signature_url=?,approved_at=?,approved_by=?,revoked_at=NULL WHERE id=?").run(certId,sig,approvedAt,req.user.id,c.id);const fresh=db.prepare('SELECT * FROM certificates WHERE id=?').get(c.id),reg=ensureRegistrationNo(c.user_id),proof=certProof(fresh,reg),fp=certFingerprint(proof);db.prepare('UPDATE certificates SET auth_token=?,auth_fingerprint=? WHERE id=?').run(proof,fp,c.id);audit(req,'certificate_approved','certificate',c.id,certId+' / '+fp);res.json({message:'Certificate approved, securely signed and issued',certificate_id:certId,registration_no:reg,auth_fingerprint:fp})});
...
app.get('/api/admin/db/tables',auth,admin,(req,res)=>{const allowed=['users','exams','passages','results','live_tests','learning_attempts','site_settings','audit_logs','otp_codes','user_exam_access','user_exam_demo_bonus','payment_requests','user_exam_blocks','user_exam_folder_blocks','gallery_items','announcements'];const out=allowed.map(name=>({name,count:db.prepare(`SELECT COUNT(*) c FROM ${name}`).get().c}));res.json(out)});
app.get('/api/admin/db/table/:name',auth,admin,(req,res)=>{const allowed=new Set(['users','exams','passages','results','live_tests','learning_attempts','site_settings','audit_logs','otp_codes','user_exam_access','user_exam_demo_bonus','payment_requests','user_exam_blocks','user_exam_folder_blocks','gallery_items','announcements']);const name=req.params.name;if(!allowed.has(name))return res.status(400).json({error:'Table not allowed'});const limit=Math.min(500,Math.max(1,Number(req.query.limit)||100));let rows=db.prepare(`SELECT * FROM ${name} ORDER BY rowid DESC LIMIT ?`).all(limit);if(name==='users')rows=rows.map(({password,...x})=>x);if(name==='otp_codes')rows=rows.map(({code_hash,...x})=>x);res.json(rows)});
...
// Normalize hosting environment values. Render values are sometimes pasted with quotes
// (for example "rzp_live_xxx"), which makes Razorpay reject otherwise valid keys.
function cleanEnvSecret(name){
 const raw=String(process.env[name]||'').trim();
 if(raw.length>=2&&((raw.startsWith('\"')&&raw.endsWith('\"'))||(raw.startsWith("'")&&raw.endsWith("'"))))return raw.slice(1,-1).trim();
 return raw;
}
const razorpayKeyId=()=>cleanEnvSecret('RAZORPAY_KEY_ID');
const razorpayKeySecret=()=>cleanEnvSecret('RAZORPAY_KEY_SECRET');
const razorpayConfigured=()=>!!(razorpayKeyId()&&razorpayKeySecret());
async function razorpayApi(pathname,opts={}){const id=razorpayKeyId(),secret=razorpayKeySecret();if(!id||!secret)throw Error('Razorpay is not configured on the server');const r=await fetch('https://api.razorpay.com/v1'+pathname,{...opts,headers:{Authorization:'Basic '+Buffer.from(id+':'+secret).toString('base64'),'Content-Type':'application/json',...(opts.headers||{})}});const data=await r.json().catch(()=>({}));if(!r.ok){if(r.status===401)throw Error('Razorpay authentication failed: check that RAZORPAY_KEY_ID and RAZORPAY_KEY_SECRET in Render belong to the same Razorpay key pair (test with test, live with live).');throw Error(data?.error?.description||'Razorpay request failed')}return data}
function paymentTarget(user,b){const kind=String(b.kind||'');if(kind==='exam'){const e0=db.prepare('SELECT * FROM exams WHERE id=? AND active=1').get(Number(b.ref_id)),e=e0?examFolderConfig(e0):null;if(!e||!e.paid_enabled)throw Error('Paid exam folder not found');return {kind,ref:String(e.id),amount:Number(e.fee_amount)||0,title:e.name,row:e}}
...
app.get('/api/razorpay/config',auth,(req,res)=>res.json({payment_enabled:paymentSystemEnabled(),configured:paymentSystemEnabled()&&razorpayConfigured(),key_id:razorpayConfigured()?razorpayKeyId():null,name:"Shivjee's Typing"}));
app.post('/api/razorpay/order',auth,async(req,res)=>{try{if(!paymentSystemEnabled())return res.status(503).json({error:'Payment system is currently OFF by Owner',code:'PAYMENT_SYSTEM_OFF'});if(!razorpayConfigured())return res.status(503).json({error:'Razorpay payment is not configured yet'});const t=paymentTarget(req.user,req.body||{});if(!(t.amount>0))return res.status(400).json({error:'Payment amount must be greater than zero'});const receipt=`sj_${req.user.id}_${t.kind}_${Date.now()}`.slice(0,40);const order=await razorpayApi('/orders',{method:'POST',body:JSON.stringify({amount:Math.round(t.amount*100),currency:'INR',receipt,notes:{user_id:String(req.user.id),kind:t.kind,ref:t.ref}})});res.json({key_id:razorpayKeyId(),order_id:order.id,amount:order.amount,currency:order.currency,name:"Shivjee's Typing",description:t.title,kind:t.kind,ref_id:t.ref})}catch(e){res.status(400).json({error:e.message||'Could not create payment order'})}});
app.post('/api/razorpay/verify',auth,async(req,res)=>{try{if(!paymentSystemEnabled())return res.status(503).json({error:'Payment system is currently OFF by Owner',code:'PAYMENT_SYSTEM_OFF'});const b=req.body||{},orderId=String(b.razorpay_order_id||''),paymentId=String(b.razorpay_payment_id||''),sig=String(b.razorpay_signature||''),kind=String(b.kind||''),ref=String(b.ref_id||'');if(!orderId||!paymentId||!sig)return res.status(400).json({error:'Incomplete payment response'});const expected=crypto.createHmac('sha256',razorpayKeySecret()).update(orderId+'|'+paymentId).digest('hex');const a=Buffer.from(expected),z=Buffer.from(sig);if(a.length!==z.length||!crypto.timingSafeEqual(a,z))return res.status(400).json({error:'Payment signature verification failed'});const t=paymentTarget(req.user,{kind,ref_id:ref});const order=await razorpayApi('/orders/'+encodeURIComponent(orderId));const payment=await razorpayApi('/payments/'+encodeURIComponent(paymentId));if(String(payment.status)!=='captured')return res.status(409).json({error:'Payment is not captured yet. Please retry shortly.'});if(String(payment.order_id||'')!==orderId)return res.status(400).json({error:'Payment order mismatch'});if(Number(order.amount)!==Math.round(t.amount*100)||Number(payment.amount)!==Math.round(t.amount*100))return res.status(400).json({error:'Payment amount mismatch'});const txn=paymentId,method='razorpay';let valid_until=null;
 db.transaction(()=>{if(kind==='exam'){const e=t.row,days=Math.max(1,Number(e.validity_days)||30);valid_until=new Date(Date.now()+days*86400000).toISOString().slice(0,10);const old=db.prepare("SELECT id FROM payment_requests WHERE user_id=? AND exam_id=? AND txn_ref=?").get(req.user.id,e.id,txn);if(!old)db.prepare("INSERT INTO payment_requests(user_id,exam_id,amount,method,txn_ref,notes,status,reviewed_at) VALUES(?,?,?,?,?,'Razorpay verified','approved',CURRENT_TIMESTAMP)").run(req.user.id,e.id,t.amount,method,txn);unblockFolderAccess(req.user.id,e);grantFolderAccess(req.user.id,e,'paid',valid_until,null)}
...
app.post('/api/analytics/visit',(req,res)=>{try{const visitor_id=String(req.body?.visitor_id||'').slice(0,120);if(!visitor_id)return res.status(400).json({error:'visitor_id required'});const page=String(req.body?.page||'home').slice(0,80),device=String(req.body?.device||'Desktop').slice(0,30),source=String(req.body?.source||'Direct').slice(0,160);db.prepare('INSERT INTO visitor_analytics(visitor_id,page,device,source) VALUES(?,?,?,?)').run(visitor_id,page,device,source);res.json({ok:true})}catch(e){res.status(500).json({error:'Analytics unavailable'})}});
app.get('/api/admin/analytics',auth,admin,(req,res)=>{const today="date('now','localtime')";const one=q=>db.prepare(q).get();const all=q=>db.prepare(q).all();res.json({totalVisitors:one('SELECT COUNT(DISTINCT visitor_id) n FROM visitor_analytics').n,todayVisitors:one(`SELECT COUNT(DISTINCT visitor_id) n FROM visitor_analytics WHERE date(created_at,'localtime')=${today}`).n,totalViews:one('SELECT COUNT(*) n FROM visitor_analytics').n,todayViews:one(`SELECT COUNT(*) n FROM visitor_analytics WHERE date(created_at,'localtime')=${today}`).n,pages:all('SELECT page,COUNT(*) n FROM visitor_analytics GROUP BY page ORDER BY n DESC LIMIT 10'),devices:all('SELECT device,COUNT(*) n FROM visitor_analytics GROUP BY device ORDER BY n DESC'),sources:all('SELECT source,COUNT(*) n FROM visitor_analytics GROUP BY source ORDER BY n DESC LIMIT 10'),days:all("SELECT date(created_at,'localtime') day,COUNT(DISTINCT visitor_id) visitors,COUNT(*) views FROM visitor_analytics WHERE created_at>=datetime('now','-6 days') GROUP BY day ORDER BY day DESC")})});

// SEO-friendly public typing landing pages (extensionless URLs)
const SEO_PUBLIC_PAGES = {
  '/typing-test':'typing-test.html',
  '/english-typing-test':'english-typing-test.html',
  '/hindi-typing-test':'hindi-typing-test.html',
  '/15-minute-typing-test':'15-minute-typing-test.html',
  '/35-wpm-typing-test':'35-wpm-typing-test.html',
  '/up-police-computer-operator-typing-test':'up-police-computer-operator-typing-test.html',
  '/ssc-typing-test':'ssc-typing-test.html',
  '/railway-typing-test':'railway-typing-test.html',
  '/wpm-calculator':'wpm-calculator.html',
  '/typing-accuracy-calculator':'typing-accuracy-calculator.html'
};
for (const [route,file] of Object.entries(SEO_PUBLIC_PAGES)) app.get(route,(req,res)=>res.sendFile(path.join(__dirname,'public',file)));
app.get('/uppco-typing-test.html',(req,res)=>res.redirect(301,'/up-police-computer-operator-typing-test'));
