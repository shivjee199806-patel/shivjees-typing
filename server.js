...
CREATE TABLE IF NOT EXISTS visitor_events(id INTEGER PRIMARY KEY AUTOINCREMENT,visitor_id TEXT NOT NULL,path TEXT NOT NULL DEFAULT '/',referrer TEXT,source TEXT NOT NULL DEFAULT 'Direct',device TEXT NOT NULL DEFAULT 'Desktop',user_id INTEGER,created_at TEXT DEFAULT CURRENT_TIMESTAMP);
...
app.post('/api/analytics/visit',(req,res)=>{try{const b=req.body||{},visitor=String(b.visitor_id||'').replace(/[^a-zA-Z0-9_-]/g,'').slice(0,80);if(visitor.length<8)return res.status(400).json({error:'Invalid visitor'});const pathName=String(b.path||'/').slice(0,180),ref=String(b.referrer||'').slice(0,300),utm=String(b.source||'').trim().slice(0,80);let source=utm||'Direct';if(!utm&&ref){try{const h=new URL(ref).hostname.toLowerCase();source=h.includes('google.')?'Google':h.includes('youtube.')||h.includes('youtu.be')?'YouTube':h.includes('instagram.')?'Instagram':h.includes('facebook.')?'Facebook':h.includes('t.me')||h.includes('telegram.')?'Telegram':h.includes('whatsapp.')?'WhatsApp':h.replace(/^www\./,'')}catch{source='Referral'}}const ua=String(req.headers['user-agent']||'').toLowerCase(),device=/ipad|tablet/.test(ua)?'Tablet':/mobi|android|iphone/.test(ua)?'Mobile':'Desktop';let uid=null;const h=req.headers.authorization||'';if(h.startsWith('Bearer ')){try{uid=jwt.verify(h.slice(7),SECRET)?.id||null}catch{}}db.prepare('INSERT INTO visitor_events(visitor_id,path,referrer,source,device,user_id) VALUES(?,?,?,?,?,?)').run(visitor,pathName,ref,source,device,uid);res.json({ok:true})}catch(e){res.status(400).json({error:'Analytics unavailable'})}});
app.get('/api/admin/analytics',auth,admin,(req,res)=>{const summary=db.prepare(`SELECT COUNT(*) page_views,COUNT(DISTINCT visitor_id) visitors,COUNT(DISTINCT CASE WHEN date(created_at,'localtime')=date('now','localtime') THEN visitor_id END) today_visitors,SUM(CASE WHEN date(created_at,'localtime')=date('now','localtime') THEN 1 ELSE 0 END) today_views FROM visitor_events`).get();const devices=db.prepare(`SELECT device,COUNT(DISTINCT visitor_id) visitors FROM visitor_events WHERE created_at>=datetime('now','-30 day') GROUP BY device ORDER BY visitors DESC`).all();const sources=db.prepare(`SELECT source,COUNT(DISTINCT visitor_id) visitors FROM visitor_events WHERE created_at>=datetime('now','-30 day') GROUP BY source ORDER BY visitors DESC LIMIT 8`).all();const pages=db.prepare(`SELECT path,COUNT(*) views,COUNT(DISTINCT visitor_id) visitors FROM visitor_events WHERE created_at>=datetime('now','-30 day') GROUP BY path ORDER BY views DESC LIMIT 10`).all();const daily=db.prepare(`SELECT date(created_at,'localtime') day,COUNT(DISTINCT visitor_id) visitors,COUNT(*) views FROM visitor_events WHERE created_at>=datetime('now','-6 day') GROUP BY day ORDER BY day`).all();res.json({summary,devices,sources,pages,daily})});
