...
app.get('/api/learning/matters/:key',auth,(req,res)=>{const key=String(req.params.key||'').slice(0,80);res.json(db.prepare('SELECT id,course_key,title,level,matter_type,content,sort_order FROM learning_matters WHERE course_key=? AND active=1 ORDER BY sort_order,id').all(key))});
