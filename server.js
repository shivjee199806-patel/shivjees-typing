...
app.post('/api/auth/google',authRateLimit,async(req,res)=>{try{
 ...
 if(u){
   ...
   const id=db.prepare("INSERT INTO users(name,email,password,role,active,plan,google_sub,referral_from) VALUES(?,?,?,'student',1,'Free',?,?)").run(name||'Candidate',email,randomPassword,sub,referral_from||null).lastInsertRowid;
