...
app.post('/api/auth/google',authRateLimit,async(req,res)=>{try{
 ...
 res.json({user:x,token:issueToken(db.prepare('SELECT id,auth_version FROM users WHERE id=?').get(x.id)),new_account:!u.created_at});
...
app.post('/api/results',auth,(req,res)=>{
 ...
 let qualOk=true,qualificationSpeed=net,qualificationFinalMistakes=null;
