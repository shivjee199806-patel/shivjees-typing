...
app.post('/api/admin/practice-matters',auth,admin,(req,res)=>{try{
 ...
 const id=db.prepare(`INSERT INTO passages(title,language,layout,difficulty,content,active,highlight_mode,exam_id,auto_scroll,practice_paid_enabled,practice_fee_amount,practice_validity_days,practice_daily_demo_limit) VALUES(?,?,?,?,?,1,'current_char',NULL,1,?,?,?,?)`).run(title,language,layout,String(b.difficulty||'Medium').slice(0,30),content,b.practice_paid_enabled?1:0,Math.max(0,Number(b.practice_fee_amount)||0),Math.max(1,Number(b.practice_validity_days)||30),Math.max(0,Number(b.practice_daily_demo_limit)||0)).lastInsertRowid;
...
app.put('/api/admin/practice-matters/:id',auth,admin,(req,res)=>{try{
 ...
 db.prepare('UPDATE passages SET title=?,content=?,language=?,layout=?,difficulty=?,practice_paid_enabled=?,practice_fee_amount=?,practice_validity_days=?,practice_daily_demo_limit=? WHERE id=?').run(title,content,language,layout,String(b.difficulty||x.difficulty||'Medium').slice(0,30),b.practice_paid_enabled===undefined?Number(x.practice_paid_enabled||0):(b.practice_paid_enabled?1:0),b.practice_fee_amount===undefined?Number(x.practice_fee_amount||0):Math.max(0,Number(b.practice_fee_amount)||0),b.practice_validity_days===undefined?Number(x.practice_validity_days||30):Math.max(1,Number(b.practice_validity_days)||30),b.practice_daily_demo_limit===undefined?Number(x.practice_daily_demo_limit||0):Math.max(0,Number(b.practice_daily_demo_limit)||0),id);audit(req,'UPDATE','practice_matter',id,`${language}: ${title}`);res.json({ok:true});
