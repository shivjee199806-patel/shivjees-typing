...
// VERIFIED REAL-EXAM BASELINES (2026-09 audit)
// IMPORTANT: never guess rules for the broad exam directory. Only rules supported by an
// authoritative recruitment notice/instruction are normalized here. Owner may override
// Backspace only; highlight is exam-profile controlled and is never an Owner/Candidate setting.
function applyVerifiedRealExamBaselines(){
 const bySlug=slug=>db.prepare('SELECT * FROM exams WHERE slug=?').get(slug);
 const setCore=(slug,vals)=>{const e=bySlug(slug);if(!e)return;const sets=[],args=[];for(const [k,v] of Object.entries(vals)){sets.push(k+'=?');args.push(v)}args.push(e.id);db.prepare(`UPDATE exams SET ${sets.join(',')} WHERE id=?`).run(...args)};
 // UPPRPB Computer Operator Grade-A: 15 min each; English 30 WPM, Hindi 25 WPM.
 // The cited Board document does not publish a separate accuracy percentage/minimum-word
 // qualification or live error-highlighting rule, so those are not invented here.
 setCore('upp-co-english',{duration:15,required_wpm:30,required_accuracy:0,min_words:0,min_chars:0,qualification_method:'wpm',highlight_mode:'none',highlight_user_change_allowed:0});
 setCore('upp-co-hindi',{duration:15,required_wpm:25,required_accuracy:0,min_words:0,min_chars:0,qualification_method:'wpm',highlight_mode:'none',highlight_user_change_allowed:0});
 // RRB NTPC CBTST: 10 min; English >=300 words/30 WPM, Hindi >=250 words/25 WPM.
 // Editing tools are prohibited. Backspace remains Owner-controlled by product requirement,
 // therefore startup MUST NOT overwrite backspace_allowed.
 const rrb=db.prepare("SELECT * FROM exams WHERE slug='railway-rrb-ntpc-typing-skill-test'").get();
 if(rrb){const hindi=String(rrb.language||'').toLowerCase()==='hindi';setCore(rrb.slug,{duration:10,required_wpm:hindi?25:30,required_accuracy:0,min_words:hindi?250:300,min_chars:0,qualification_method:'words_wpm_accuracy',highlight_mode:'none',highlight_user_change_allowed:0})}
 // SSC CHSL LDC/JSA: 10 min; English 35 WPM / Hindi 30 WPM. No unsupported accuracy %.
 const chsl=bySlug('ssc-chsl-ldc-jsa-typing');if(chsl){const hindi=String(chsl.language||'').toLowerCase()==='hindi';setCore(chsl.slug,{duration:10,required_wpm:hindi?30:35,required_accuracy:0,min_words:0,min_chars:0,qualification_method:'wpm',highlight_mode:'none',highlight_user_change_allowed:0})}
 // Delhi Police HCM 2025: 10 min; English 30 WPM / Hindi 25 WPM. Passage size is supplied
 // by the authority, but qualification is speed based, so passage length is not used as a second pass gate.
 const dph=bySlug('delhi-police-hcm-typing');if(dph){const hindi=String(dph.language||'').toLowerCase()==='hindi';setCore(dph.slug,{duration:10,required_wpm:hindi?25:30,required_accuracy:0,min_words:0,min_chars:0,qualification_method:'wpm',highlight_mode:'none',highlight_user_change_allowed:0})}
 // Highlight is not user-configurable. Unknown/unverified folders stay conservative OFF rather
 // than pretending a visual behaviour is official. A verified exam profile can explicitly set it later.
 db.prepare("UPDATE exams SET highlight_user_change_allowed=0,highlight_mode='none' WHERE slug NOT LIKE 'live-template-%'").run();
...
applyVerifiedRealExamBaselines();
...
app.post('/api/razorpay/verify',auth,async(req,res)=>{try{const b=req.body||{},orderId=String(b.razorpay_order_id||''),paymentId=String(b.razorpay_payment_id||''),sig=String(b.razorpay_signature||''),kind=String(b.kind||''),ref=String(b.ref_id||'');if(!orderId||!paymentId||!sig)return res.status(400).json({error:'Incomplete payment response'});const expected=crypto.createHmac('sha256',String(process.env.RAZORPAY_KEY_SECRET||'')).update(orderId+'|'+paymentId).digest('hex');const a=Buffer.from(expected),z=Buffer.from(sig);if(a.length!==z.length||!crypto.timingSafeEqual(a,z))return res.status(400).json({error:'Payment signature verification failed'});const t=paymentTarget(req.user,{kind,ref_id:ref});let order=await razorpayApi('/orders/'+encodeURIComponent(orderId));let payment=await razorpayApi('/payments/'+encodeURIComponent(paymentId));
 ...
 if(String(order.status)!=='paid'&&Number(order.amount_paid)!==expectedAmount)return res.status(409).json({error:'Razorpay order is not marked paid yet. Please retry shortly.'});
