...
app.post('/api/auth/google',authRateLimit,async(req,res)=>{try{
 ...
 if(u){
   ...
   return res.json({user:x,token:jwt.sign({id:x.id},SECRET,{expiresIn:'7d'}),new_account:false});
 }
 if(setting('allow_registration')!=='1')return res.status(403).json({error:'New registration is currently disabled'});
 const profile_token=jwt.sign({purpose:'google_profile',sub,email,name},SECRET,{expiresIn:'15m'});
 return res.json({needs_profile:true,profile_token,email,name});
 }catch(e){console.warn('Google login:',e.message);res.status(503).json({error:'Google Login is temporarily unavailable'})}});
app.post('/api/auth/google-complete',authRateLimit,(req,res)=>{try{
 const token=String(req.body?.profile_token||''),name=String(req.body?.name||'').trim().replace(/\s+/g,' ').slice(0,80),phone=normalizePhone(req.body?.phone),password=String(req.body?.password||'');
 let g;try{g=jwt.verify(token,SECRET)}catch(_){return res.status(401).json({error:'Google registration session expired. Please use Google Login again.'})}
 if(g.purpose!=='google_profile'||!g.sub||!emailOk(String(g.email||'')))return res.status(401).json({error:'Invalid Google registration session'});
 if(name.length<2)return res.status(400).json({error:'Enter your full name'});if(!phone)return res.status(400).json({error:'Enter a valid 10-digit Indian mobile number'});if(password.length<8)return res.status(400).json({error:'Password must be at least 8 characters'});
 let u=db.prepare('SELECT * FROM users WHERE google_sub=? OR lower(trim(email))=?').get(g.sub,String(g.email).toLowerCase());
 if(u){if(!u.google_sub)db.prepare('UPDATE users SET google_sub=? WHERE id=?').run(g.sub,u.id)}else{
   const phoneUsed=db.prepare('SELECT id FROM users WHERE phone=?').get(phone);if(phoneUsed)return res.status(409).json({error:'This mobile number is already registered. Login with your existing account.'});
   const id=db.prepare("INSERT INTO users(name,email,phone,password,role,active,plan,google_sub,phone_verified) VALUES(?,?,?,?, 'student',1,'Free',?,0)").run(name,String(g.email).toLowerCase(),phone,bcrypt.hashSync(password,10),g.sub).lastInsertRowid;u=db.prepare('SELECT * FROM users WHERE id=?').get(id);
 }
 if(u.role==='admin'||Number(u.is_owner)===1)return res.status(403).json({error:'Owner/Admin accounts must use Owner Login'});db.prepare('UPDATE users SET last_login=CURRENT_TIMESTAMP,login_count=COALESCE(login_count,0)+1 WHERE id=?').run(u.id);const x=safe(db.prepare('SELECT * FROM users WHERE id=?').get(u.id));res.json({user:x,token:jwt.sign({id:x.id},SECRET,{expiresIn:'7d'}),new_account:true});
 }catch(e){res.status(400).json({error:e.message||'Could not create Google account'})}});
...
app.get('/api/razorpay/config',auth,(req,res)=>res.json({configured:razorpayConfigured(),key_id:razorpayConfigured()?String(process.env.RAZORPAY_KEY_ID):null,name:"Shivjee's Typing"}));
...
app.get('/api/admin/analytics',auth,admin,(req,res)=>{const all=q=>db.prepare(q).all(),one=q=>db.prepare(q).get();res.json({totalVisitors:one('SELECT COUNT(DISTINCT visitor_id) n FROM visitor_analytics').n,todayVisitors:one("SELECT COUNT(DISTINCT visitor_id) n FROM visitor_analytics WHERE date(created_at,'localtime')=date('now','localtime')").n,totalViews:one('SELECT COUNT(*) n FROM visitor_analytics').n,todayViews:one("SELECT COUNT(*) n FROM visitor_analytics WHERE date(created_at,'localtime')=date('now','localtime')").n,pages:all('SELECT page,COUNT(*) n FROM visitor_analytics GROUP BY page ORDER BY n DESC LIMIT 10'),devices:all('SELECT device,COUNT(*) n FROM visitor_analytics GROUP BY device ORDER BY n DESC'),sources:all('SELECT source,COUNT(*) n FROM visitor_analytics GROUP BY source ORDER BY n DESC LIMIT 10'),days:all("SELECT date(created_at,'localtime') day,COUNT(DISTINCT visitor_id) visitors,COUNT(*) views FROM visitor_analytics WHERE created_at>=datetime('now','-6 days') GROUP BY day ORDER BY day DESC")})});
