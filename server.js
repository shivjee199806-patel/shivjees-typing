...
// Razorpay credentials: normalize accidental whitespace/newlines from dashboard/env copy-paste.
// Never log or return the secret.
const razorpayCredentials=()=>({
  id:String(process.env.RAZORPAY_KEY_ID||'').replace(/\s+/g,''),
  secret:String(process.env.RAZORPAY_KEY_SECRET||'').replace(/\s+/g,'')
});
const razorpayConfigured=()=>{const {id,secret}=razorpayCredentials();return !!(id&&secret)};
async function razorpayApi(pathname,opts={}){const {id,secret}=razorpayCredentials();if(!id||!secret)throw Error('Razorpay is not configured on the server');const r=await fetch('https://api.razorpay.com/v1'+pathname,{...opts,headers:{Authorization:'Basic '+Buffer.from(id+':'+secret).toString('base64'),'Content-Type':'application/json',...(opts.headers||{})}});const data=await r.json().catch(()=>({}));if(!r.ok){const desc=data?.error?.description||'Razorpay request failed';console.error('[Razorpay API]',r.status,pathname,desc);throw Error(desc)}return data}
