...
app.post('/api/auth/google',authRateLimit,async(req,res)=>{try{
 ...
 res.json({user:x,token:jwt.sign({id:x.id},SECRET,{expiresIn:'7d'}),new_account:isNewGoogleAccount,google_profile_required:isNewGoogleAccount&&Number(x.google_profile_completed||0)!==1});
...
app.patch('/api/me/google-first-profile',auth,(req,res)=>{try{
 ...
 if(!u||u.role==='admin')return res.status(403).json({error:'Candidate account required'});
 if(!u.google_sub)return res.status(403).json({error:'This setup is only for Google signup candidates'});
 if(Number(u.google_profile_completed||0)===1)return res.status(409).json({error:'Google first-time profile is already completed'});
 ...
 const taken=db.prepare('SELECT id FROM users WHERE id<>? AND phone=? LIMIT 1').get(u.id,phone);if(taken)return res.status(409).json({error:'This mobile number is already registered'});
 db.prepare('UPDATE users SET name=?,phone=?,password=?,phone_verified=0,google_profile_completed=1 WHERE id=?').run(name,phone,bcrypt.hashSync(password,10),u.id);
 const x=safe(db.prepare('SELECT * FROM users WHERE id=?').get(u.id));res.json({ok:true,user:x,login_ids:{email:x.email,phone:x.phone}});
 }catch(e){console.error('Google first profile:',e);res.status(500).json({error:'Could not save Google login details'})}});
